...
global_feature_headers = [
    "sample",
    *[f"contour_hinge_{i}" for i in range(300)],
]
...
pca_feature_headers = [
    "sample",
    *[f"contour_hinge_pca_{i}" for i in range(num_components)],
]
...
os.makedirs(f"{c.RAW_FEATURES}/{c.PCA}/contour_hinge", exist_ok=True)
...
def process_line(writer_folder, line_path):
    ...
    _, binary_line = cv2.threshold(
        line, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU
    )
    binary_line_inv = cv2.bitwise_not(binary_line)
    countour_hinge_hist = contour_hinge_pdf(binary_line_inv)

    data = pd.DataFrame([[f"{writer_folder}_{sample_id}_{speed}_{line_num}", *countour_hinge_hist]])
    with lock:
        data.to_csv(
            f"{c.RAW_FEATURES}/{c.HIST}/contour_hinge_hist.csv",
            mode="a",
            header=False,
            index=False,
        )
...
def process_line_pca(writer_folder, line_path, pca, scaler):
    ...
    contour_hinge_hist = scaler.transform([contour_hinge_hist])
    contour_hinge_hist = pca.transform(contour_hinge_hist)
    contour_hinge_hist = contour_hinge_hist.flatten()

    data = pd.DataFrame([[f"{writer_folder}_{sample_id}_{speed}_{line_num}", *contour_hinge_hist]])
    with lock:
        data.to_csv(
...
for writer_folder in tqdm(writers, desc="Extracting Contour Hinge Features"):
    process_writer_global(writer_folder)

data = pd.read_csv(f"{c.RAW_FEATURES}/{c.HIST}/contour_hinge_hist.csv")
...
joblib.dump(pca, f'{c.MODEL_FOLDER}/{c.PCA}/contour_hinge_hist_pca.pkl')
joblib.dump(scaler, f'{c.MODEL_FOLDER}/{c.PCA}/contour_hinge_hist_scaler.pkl')
