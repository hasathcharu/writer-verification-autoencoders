...
from modules.preprocess.preprocess import read_image, binarize_only
...
ocr = VisionEncoderDecoderModel.from_pretrained("microsoft/trocr-large-handwritten").to(
    device
)


def classify_words(lines):
    and_words = []
    the_words = []
    words = segment_words(lines, word_threshold=44000, min_consecutive=20)

    for word in words:
        word_c = cv2.cvtColor(word, cv2.COLOR_BGR2RGB)
        word_c = Image.fromarray(word_c)
        pixel_values = processor(images=word_c, return_tensors="pt").pixel_values.to(
            device
        )
        ...
        if extracted_text == c.AND:
            and_words.append(word)
        elif extracted_text == c.THE:
            the_words.append(word)
    return and_words, the_words
...
def preprocess(writer_folder, img_file):
    ...
    (lines, dirty_lines), cropped_rectangles = segment_lines(
        binary_image, threshold=380000, line_height=180, size=1600
    )
    dirty_raw_lines = [
        raw_image[y1:y2, x1:x2] for (x1, y1, x2, y2) in cropped_rectangles
    ]
    raw_lines, _ = clean_lines(dirty_raw_lines)
    and_words, the_words = classify_words(lines)
    ...
    with lock:
        ...
        for i, word in enumerate(and_words):
            cv2.imwrite(
                f"{WORD_OUTPUT_PATH}/{writer_folder}/{c.AND}/{sample_id}/{sample_id}_{speed}_{i}.png",
                word,
            )
        for i, word in enumerate(the_words):
            cv2.imwrite(
                f"{WORD_OUTPUT_PATH}/{writer_folder}/{c.THE}/{sample_id}/{sample_id}_{speed}_{i}.png",
                word,
            )
...
with tqdm(
    ...
    for writer_folder in writer_folders:
        ...
        with ThreadPoolExecutor(max_workers=N_CORES * 2) as img_executor:
            futures = [
