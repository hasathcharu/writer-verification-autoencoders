...
os.makedirs(f"{c.FEATURES}/{c.TRAIN}/", exist_ok=True)
os.makedirs(f"{c.FEATURES}/{c.VAL}/", exist_ok=True)
os.makedirs(f"{c.FEATURES}/{c.TEST}/", exist_ok=True)
os.makedirs(f"{c.FEATURES}/{c.VAL_OWN}/", exist_ok=True)
os.makedirs(f"{c.FEATURES}/{c.PRETRAINED}/", exist_ok=True)

and_columns = ["and_presence"]
the_columns = ["the_presence"]
...
pretrain_writers = [line.strip() for line in open(f'{c.FEATURES}/pretrain.txt', 'r')]

pre_train = pd.DataFrame()


for writer in tqdm(pretrain_writers, desc="Preparing Pretrained Data"):
    df = pd.read_csv(f"{c.RAW_FEATURES}/{c.GLOBAL}/{writer}.csv")
    if df.shape[0] < 3:
        print(f"[PRETRAIN] Skipping {writer} due to insufficient data")
        continue

    pre_train = pd.concat([pre_train, df])

pre_train.to_parquet(f"{c.FEATURES}/{c.PRETRAINED}/data.parquet", index=False)
...
for writer in tqdm(writers, desc="Preparing Global Data"):
    ...
    train.to_parquet(f"{c.FEATURES}/{c.TRAIN}/{writer}.parquet", index=False)
    ...
    val_own.to_parquet(f"{c.FEATURES}/{c.VAL_OWN}/{writer}.parquet", index=False)
    val.to_parquet(f"{c.FEATURES}/{c.VAL}/{writer}.parquet", index=False)
    test.to_parquet(f"{c.FEATURES}/{c.TEST}/{writer}.parquet", index=False)
