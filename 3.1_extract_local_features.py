...
def get_local_features(binary_char):
    num_black_pixels = get_num_of_black_pixels(binary_char)
    binary_char_inv = cv2.bitwise_not(binary_char)
    largest_curve_area, largest_interior_area = get_contour_areas(binary_char_inv)
    chaincode_histogram, chaincode_images, color_chaincode_image = (
        compute_chaincode_histogram(binary_char_inv)
    )
    stroke_width_hist, stroke_widths = compute_stroke_width_histogram(binary_char_inv)
    hod = compute_hod(binary_char)
    data = [
        num_black_pixels,
        largest_curve_area,
        largest_interior_area,
        # *chaincode_histogram,
        np.mean(stroke_widths),
